                ...
                <div className="container w-full p-4 md:px-14 pt-2">
                  <div className="flex justify-space-between grid gap-1 md:gap-2 grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                    {searchQuery ? (
                      ...
                      data.results.slice(pager, pager+24).map((pokemon: PokemonEntry) => (
                        <div className="cursor-pointer" key={pokemon.name} onClick={() => dispatch(setTarget({name: pokemon.name}))} data-name={pokemon.name}>
                          <PokemonTile pokemon={pokemon} />
                        </div>
                      ))
                    ...
                    <div className="container flex w-full justify-between">
                      {!searchQuery && pager > 24 && 
                        <div className="flex text-black text-2xl cursor-pointer flex-nowrap">
                          <button onClick={() => setPager(pager-25)} className="inline btn btn-md text-nowrap">&laquo; Previous</button>
                        </div>
                      }
                      {!searchQuery &&
                        <div className="flex text-black text-2xl cursor-pointer flex-nowrap">
                          <button onClick={() => setPager(pager+25)} className="inline btn btn-md text-nowrap">Next &raquo;</button></div>
                      }
                    </div>
                  </div>
                </div>
