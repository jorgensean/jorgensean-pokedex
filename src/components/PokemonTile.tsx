...
import type { PokemonEntry, PokemonType } from "../types/pokemon"
...
export const PokemonTile: React.FC<Props> = ({ pokemon }) => {
  ...
  const primaryType: string =
    "border-[6px] border-amber-300 rounded-xl min-w-40 min-h-52 relative pokemon-tile".concat(
  ...
  return (
    <div className={primaryType}>
      <h2
        className={"capitalize rounded-t-lg font-bold text-white mb-5 bg-black bg-opacity-50 ".concat(
          pokemon.name.length < 14
            ? "py-1 pl-4 text-lg"
            : "text-center py-2 pl-0.5 text-xs",
        )}
      >
        {pokemon.name}
        <span className="text-6xl absolute bottom-3/4 right-0 text-white text-opacity-25">
          #{data.id}
        </span>
      </h2>
      <img
        ...
        {data.types.length
          ? data.types.map((type: PokemonType) => (
              <div
                ...
                className={"py-1 px-0.5 rounded-xl text-white text-center text-sm capitalize "
                  // add bg filter or bg-<type> if secondary
                  .concat(
                    type.type.name !== data.types[0].type.name
                      ? `bg-${data.types[1].type.name}`
