...
export const PokemonDetail: React.FC<Props> = ({ pokemon }: Props) => {
  ...
  const primaryType: string = "capitalize text-5xl 2xl:text-7xl py-3 pl-16 outline outline-1 outline-transparent -rotate-y-[7deg] -translate-x-16 border-t border-b border-black scale-105 text-white overflow-hidden".concat(data.types.length ? ` bg-${data.types[0].type.name}` : "")
  const secondaryType: string = "pill capitalize".concat(data.types.length > 1 ? ` bg-${data.types[1].type.name}` : "")
      ...
      <div className="container w-full flex px-6 pt-2">
        <div className="flex-none w-1/2 text-left">
          {data.sprites.other["official-artwork"].front_default && (
            <img className="max-w-60" src={data.sprites.other["official-artwork"].front_default} alt={data.name} />
          )}
        </div>
        <div className="grow"></div>
        <div className="flex-none w-1/2">
          {data.species.url && (
            <PokemonDescription url={data.species.url} />
          )}
            ...
            <div className="grid gap-2 grid-cols-2">
              {data.types.length ? 
                data.types.map((type: PokemonType) => (
                  <div key={`${pokemon.name}-${type.type.name}`} className={"py-1 px-0 rounded-lg text-white text-center text-md capitalize".concat(` bg-${type.type.name}`)}>
      ...
      <div className="container w-full flex p-4 pt-2">
        ...
        {data.stats.length ? 
          data.stats.map((stats: PokemonStats) => (
            <div key={`${pokemon.name}-${stats.stat.name}`} className="text-white text-md capitalize">
