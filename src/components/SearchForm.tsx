...
import { unsetTarget, selectedPokemonValue } from "../features/pokemon/selectedPokemonSlice"
import { setSearchQuery, clearSearch } from "../features/pokemon/pokemonSearchSlice"
import { addToHistory, clearItem, pokemonSearchHistory } from "../features/pokemon/pokemonSearchHistorySlice"
    ...
    <div className="mx-auto">
      <form onSubmit={formSubmit} autoComplete="off">
        <div className="container mt-2 mb-0 mx-auto w-3/4 flex items-center py-2 px-8 rounded-xl bg-white">
          ...
          {searchTerm ? (<button type="button" onClick={resetSearch} className="inline py-2 px-4 mt-0.5 mr-4 btn bg-gray-500">Reset</button>) : ''}
          <button className="flex-shrink-0 bg-teal-500 uppercase font-medium hover:bg-teal-700 border-teal-500 hover:border-teal-700 text-sm border-4 text-white py-1 px-2 rounded-lg" type="submit">
            Search
          </button>
        </div>
        ...
        {data ? 
          <div className="w-full lg:w-1/3 px-6 rounded-b-xl absolute text-black text-left z-10 right-full lg:right-96 xl:right-1/3 2xl:right-1/3">
            <div className="ml-5 mr-12 px-3 py-0 rounded-b-xl border-r border-l border-b border-gray-400 shadow-lg bg-white">
              {allowAutocomplete && searchHistory.length && !selectedPokemon ? (searchHistory.map((item: string) => (
                <div className="flex" key={item}>
                  <div className="capitalize w-3/4 float-left" data-name={item} onClick={autofillSearch}>
                    {item}
                  </div>
                  <div className="capitalize w-1/4 float-right">
                    <button onClick={removeSearchItem} data-name={`${item}-${nanoid}`} key={item.concat('-remove')} className="text-xs btn btn-sm float-right">&times; Remove</button>
                  </div>
                </div>
              ))) : ''}
              ...
              .map((pokemon: PokemonEntry) => (
                <div className="capitalize w-1/2 last:pb-2" data-name={pokemon.name} key={pokemon.name} onClick={autofillSearch}>{pokemon.name}</div>
              ))}
            </div>
          </div>
        : ''}
